...
static void refresh_values(lv_obj_t* content)
{
    ...
    lv_label_set_text(lv_obj_get_child(row_sound, 1), snd?"On":"Off");
}
...
void settings_menu_screen_create(lv_obj_t* parent)
{
    ...
    make_row(content, "Step Goal", "--", open_goal);
    make_row(content, "Display Timeout", "--", open_timeout);
    make_row(content, "Sound", "--", open_sound);
    make_row(content, "Storage", "Tools", open_storage);

    refresh_values(content);
}
